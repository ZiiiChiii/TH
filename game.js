...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  // 【修正 2】將起始 3D 動畫改為從 rotateY(0deg) 開始
  animCard.style.transform = 'perspective(800px) rotateY(0deg) scale(1)';
  ...
  setTimeout(() => {
    ...
    // 【修正 2】將飛行結束狀態改為 rotateY(180deg) 並適當微調縮放值
    animCard.style.transform = `perspective(800px) translate(${deltaX}px, ${deltaY}px) rotateY(180deg) scale(0.15)`;
