...
// ==========================================
// 3. 【修正 2】優化後的 GSAP 3D 動態抓取終點與拋物飛行特效
// ==========================================
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  // 【修正 2】如果找不到動態生成的 vault-target 元素，退而使用 dashboard 或 body 當終點
  ...
  .to(flyCard, {
    ...
    onComplete: () => {
      ...
      // 【修正 2】Bounce 彈跳特效僅在真實的金庫元素存在時才執行
      if (vaultDom) {
...
function renderDashboardGems(targetElementId, actorData, diffs) {
  ...
  ['w', 'u', 'g', 'r', 'k', 'o'].forEach(k => {
    ...
    // 【修正 3】補齊並確認每個顏色的 res-block 都有被正確設定 id="vault-target-${k}"
    html += `
