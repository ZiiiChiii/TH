...
// 將原陣列改為 Map 結構，以 cardId 為 key 管理 tween
...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  if (!sourceDom) { 
    console.warn(`[animateCardFlightToGoldVault] 找不到來源元素: dom-card-${cardId}`);
  ...
  .to(flyCard, {
    ...
    onComplete: () => {
      ...
      // 【問題二修復】先執行同步回調（含更新與 render），最後將 flag 設回 false
      if (callback) callback();
      isAnimating = false;

      // 【問題二修復：Bug B】強制同步更新一次狀態快照，阻絕殘留 diff 導致下一回合選籌碼時重複撥放
      lastPlayerState = deepClone(CoreState.get().player);
    }
  });
}
...
window.render = function() {
  ...
  ['lv1', 'lv2', 'lv3'].forEach(level => {
    ...
    const boardCards = fullState.board[level];

    boardCards.forEach((card, idx) => {
      const cardId = card ? `dom-card-${card.id}` : `empty-${level}-${idx}`;
      ...
      if (!card) {
        if (!el || !el.classList.contains('empty')) {
          ...
          emptyDiv.id = `empty-${level}-${idx}`;
          emptyDiv.textContent = '已全數售罄';
          
          const children = row.children;
          if (children[idx]) row.replaceChild(emptyDiv, children[idx]);
          else row.appendChild(emptyDiv);
        }
        return;
      }
      ...
      if (!el) {
        ...
        for (let k in card.cost) {
          costHtml += `<div class="cost-dot ${(player.bonus[k]||0) >= card.cost[k] ? 'free' : ''}">
            <span class="cost-dot-circle ${GEM_CLASSES[k]}"></span>
            <span>${card.cost[k]}</span></div>`;
        }
        ...
        div.className = 'card animate-deal';
        div.id = cardId;
          ...
          <div class="card-content-wrapper">
            ...
            <div>
              ...
              <div class="card-actions">
                <button class="btn-card buy-btn" ${!isPlayerTurn || !afford.affordable ? 'disabled' : ''} onclick="buyBoardCard('${level}', ${idx})">收購</button>
                <button class="btn-card reserve-btn" ${!isPlayerTurn || player.reserved.length >= 3 ? 'disabled' : ''} onclick="reserveBoardCard('${level}', ${idx})">保留</button>
              </div>
            </div>
          </div>`;
        
        const children = row.children;
        if (children[idx]) row.replaceChild(div, children[idx]);
        ...
        if (buyBtn) buyBtn.disabled = !isPlayerTurn || !afford.affordable;
        if (reserveBtn) reserveBtn.disabled = !isPlayerTurn || player.reserved.length >= 3;

        el.querySelectorAll('.cost-dot').forEach((dot, di) => {
          const costKeys = Object.keys(card.cost);
          if (costKeys[di]) {
            const k = costKeys[di];
            dot.classList.toggle('free', (player.bonus[k]||0) >= card.cost[k]);
          }
        });
      }
    });
  ...
  // 【問題一修復 ①】延遲一個 frame 執行 Idle 漂浮，避開與入場 animate-deal CSS 核心 Transform 產生相互覆蓋衝突
  requestAnimationFrame(() => setupIdleCardAnimations());
}

// 【問題一修復 ②】強健的 Map 差異比對與死亡/無效狀態判定機制
function setupIdleCardAnimations() {
  ...
  document.querySelectorAll('.board-matrix .card:not(.empty)').forEach((el, i) => {
    const cardId = el.id || '';
    if (!cardId) return;
    if (activeFlyingCardIds.has(cardId.replace('dom-card-', ''))) return;
    
    currentCardIds.add(cardId);
    
    // 差異化檢查：已有動畫時進行活躍度安全檢驗
    if (window._idleTweensMap.has(cardId)) {
      const existingTween = window._idleTweensMap.get(cardId);
      if (!existingTween || existingTween.killed || (!existingTween.isActive() && existingTween.totalProgress() === 0)) {
        window._idleTweensMap.delete(cardId);
      } else {
        return; // 動態正常，安全跳過不重複建立
      }
    }
