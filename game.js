...
let isAnimating = false; // 🚀 補回動畫狀態鎖

window._idleTweensMap = window._idleTweensMap || new Map(); // 🚀 改用高效且不衝突的 Map 機制控制浮空
...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  isAnimating = true; // 🚀 開啟鎖定，避免重繪中斷
  activeFlyingCardIds.add(cardId);

  // 移除該卡片上原有的浮空 idle 動畫，避免多重位移衝突
  ...
  .to(flyCard, {
    ...
    onComplete: () => {
      ...
      lastPlayerState = deepClone(CoreState.get().player); // 🚀 飛完才刷新基準狀態，確保 +1 數字能正常抓到
      render();
    }
  });
}
...
window.render = function() {
  ...
  // 🚀 關鍵修復：只有在沒有進行飛牌動畫時，才去計算和更新 diff 狀態，避免跳轉
  ...
  if (!isAnimating) {
    lastPlayerState = deepClone(player); // 🚀 只有在動畫未鎖定狀態，才同步克隆基準值
  }
  ...
  ['lv1', 'lv2', 'lv3'].forEach(level => {
    ...
    // 🚀 改回標準 innerHTML 渲染，但配合 id 快取，防止飛牌過程中骨架被重刷
...
function setupIdleCardAnimations() {
  ...
  document.querySelectorAll('.board-matrix .card:not(.empty)').forEach((el, i) => {
    ...
    if (activeFlyingCardIds.has(pureId)) return; // 如果正在飛行，跳過浮空
