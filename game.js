...
// 【問題一修復】將原陣列改為 Map 結構，以 cardId 為 key 管理 tween
...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  // 【問題一修復】從 Map 中精準尋找並清除該張卡牌舊的待命動畫
  ...
  .to(flyCard, {
    ...
    onComplete: () => {
      ...
      // 【問題二修復】先執行同步回調（含更新與 render），最後才將 flag 設回 false
      if (callback) callback();
      isAnimating = false;
    }
  });
...
window.render = function() {
  ...
  // 確保每次 DOM 渲染完成後呼叫差異化待命動畫設定
  setupIdleCardAnimations();
}

// 【問題一修復】帶有「差異比對」邏輯的全新 setupIdleCardAnimations 函式
function setupIdleCardAnimations() {
  ...
  // 清除已經離開桌面場上的卡牌動畫資源（例如剛被玩家收購或保留的卡牌）
...
// 【問題三修復】將錯誤的增加 class 改回正確的關閉 (remove) 視窗
