...
window.render = function() {
  ...
  // ==========================================
  // 【修正區】全自動卡牌序列比對，防止每次點擊都中斷 idle 呼吸
  // ==========================================
  // 取得目前在矩陣上的所有卡牌元素
  const currentCards = Array.from(
    document.querySelectorAll('.board-matrix .card:not(.empty)')
  );
  const currentIds = currentCards.map(el => el.id).join(',');

  // 只有卡牌組合有變動時才重新建立 tween
  if (currentIds !== window._lastIdleCardIds) {
    window._lastIdleCardIds = currentIds;
    ...
    currentCards.forEach((el, i) => {
      // 安全機制：若該卡正處於飛行動畫鎖定狀態，跳過不重綁 idle
