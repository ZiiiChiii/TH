...
window.render = function() {
  ...
  // ==========================================
  // 【修正 1】將卡牌矩陣重構為不損壞 GSAP 節點的「局部動態更新」流程
  // ==========================================
  ['lv1', 'lv2', 'lv3'].forEach(level => {
    ...
    boardCards.forEach((card, idx) => {
      ...
      if (!el) {
          ...
          <div class="card-content-wrapper">
            ...
            <div>
              ...
              <div class="card-actions">
                <button class="btn-card buy-btn" onclick="buyBoardCard('${level}', ${idx})">收購</button>
                <button class="btn-card reserve-btn" onclick="reserveBoardCard('${level}', ${idx})">保留</button>
              </div>
            </div>
          </div>`;
        ...
        // 已存在的卡牌：只更新按鈕 disabled 狀態和費用顏色，不重建 DOM
        const afford = GameEngine.canAffordCard(player.bonus, player.tokens, card.cost);
