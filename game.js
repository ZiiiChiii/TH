...
window.render = function() {
  ...
  ['lv1', 'lv2', 'lv3'].forEach(level => {
    ...
    document.getElementById(`row-${level}`).innerHTML = fullState.board[level].map((card, idx) => {
      ...
      for (let k in card.cost) {
        const isFree = (player.bonus[k]||0) >= card.cost[k] ? 'free' : '';
        costHtml += '<div class="cost-dot ' + isFree + '"><span class="cost-dot-circle ' + GEM_CLASSES[k] + '"></span><span>' + card.cost[k] + '</span></div>';
      }
      const afford = GameEngine.canAffordCard(player.bonus, player.tokens, card.cost);
      const imgUrl = CUSTOM_CARD_IMAGES[card.provides][parseInt(card.id) % CUSTOM_CARD_IMAGES[card.provides].length];
      const dealClass = !lastRenderedCardIds.has(card.id) ? 'animate-deal' : '';
      const buyDis = (!isPlayerTurn || !afford.affordable) ? 'disabled' : '';
      const resDis = (!isPlayerTurn || player.reserved.length >= 3) ? 'disabled' : '';
      return '<div class="card ' + dealClass + '" id="dom-card-' + card.id + '" style="background-image: url(\'' + imgUrl + '\');">'
        + '<div class="card-content-wrapper">'
        + '<div class="card-top"><span class="card-pts">' + (card.points > 0 ? card.points : '') + '</span>'
        + '<div class="card-gem-icon ' + GEM_CLASSES[card.provides] + '"></div></div>'
        + '<div><div class="card-costs">' + costHtml + '</div>'
        + '<div class="card-actions">'
        + '<button class="btn-card" ' + buyDis + ' onclick="buyBoardCard(\'' + level + '\', ' + idx + ')">收購</button>'
        + '<button class="btn-card" ' + resDis + ' onclick="reserveBoardCard(\'' + level + '\', ' + idx + ')">保留</button>'
        + '</div></div></div></div>';
    }).join('');
  });
  ...
  // idle tween：每次 render 重建，用 gsap.isTweening 跳過已在動的卡
  document.querySelectorAll('.board-matrix .card:not(.empty)').forEach((el, i) => {
    ...
    if (window.activeFlyingCardIds && window.activeFlyingCardIds.has(cardIdAttr)) return;
    gsap.to(el, {
      y: -6,
      rotation: 0.8,
