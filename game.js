...
function animateCardFlightToGoldVault(cardId, providesColor, callback) {
  ...
  // 加上透視感並設定初始不加 transition 的 transform
  animCard.style.perspective = '800px';
  ...
  setTimeout(() => {
    ...
    // 用單一 transform 字串同時完成平移、縮小、與 3D 翻轉
    animCard.style.transform = `translate(${deltaX}px, ${deltaY}px) scale(0.2) rotateY(360deg)`;
    animCard.style.opacity = '0';
  }, 150);

  // ── 第三階段：動畫結束處理 (150ms + 650ms) ──
  setTimeout(() => {
    animCard.remove(); // 從圖層中移除動畫卡
    sourceDom.style.transform = ''; // 恢復原始卡牌狀態
    sourceDom.style.zIndex = '';

    // 觸發金庫/背包的 Bounce 彈跳特效
    targetDom.classList.remove('vault-bounce');
    void targetDom.offsetWidth; // 強制 Reflow 重新播放動畫
